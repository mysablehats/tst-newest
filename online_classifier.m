function aoutout = online_classifier(gas, allskel, arq_connect, simvar)
...
%     [data.test.gas(i).class, ~] = find(gas(i).nodesl(:, data.test.gas(i).bestmatchbyindex));
% end
...
if 0% simvar.paramsZ.PLOTIT
    figure
    plot_act(data, datatest.data, simvar.paramsZ.skelldef, numskells) %% data.test!!!!
end
